...
void		ft_display_message2(t_three *philo, int type, int n)
{
	char *str;
...
void		ft_display_message(t_three *philo, int type, size_t n)
{
	...
	sem_wait(philo->info->write_m);
	sem_wait(philo->info->dead_write_m);
	if (z == 'z')
	{
	...
	if (type < 4)
		sem_post(philo->info->dead_write_m);
	sem_post(philo->info->write_m);
}
